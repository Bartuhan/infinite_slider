...
class _DragableWidgetState extends State<DragableWidget> with SingleTickerProviderStateMixin {
  ...
  double get currentAngle {
    return currentPosition.dx < 0
        ...
        : currentPosition.dx + size.width > screenSize.width
            ? (pi * 0.2) * (currentPosition.dx + size.width - screenSize.width) / size.width
            : 0;
  }
  ...
  void initState() {
    ...
    )..addListener(restoreAnimtionListener);
  ...
  void dispose() {
    restoreController
      ..removeListener(restoreAnimtionListener)
      ..dispose();
    super.dispose();
  }
